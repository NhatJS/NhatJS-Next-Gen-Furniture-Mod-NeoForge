...
public class LightRenderer implements BlockEntityRenderer<LightBlockEntity, LightRenderState> {
    ...
    public void submit(LightRenderState renderState, PoseStack matrices, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
        ...
        submitNodeCollector.submitBlockModel(matrices, RenderType.cutoutMipped(), led, 1, 1, 1, renderState.light, renderState.overlay, 0);
        if (renderState.powered) {
            submitNodeCollector.submitBlockModel(matrices, RenderType.cutoutMipped(), led, 1, 1, 1, LightTexture.FULL_BRIGHT, renderState.overlay, 0);
        }
        matrices.popPose();
    }
}